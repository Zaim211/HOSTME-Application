...
export default function PlacesFormPage() {
  ...
  return (
    <div>
      <AccountNav />
      <form onSubmit={savePlace} className="mb-7">
        ...
        <textarea value={description} className="textarea-field" onChange={ev => setDescription(ev.target.value)} />
        {preInput('Perks','select all the perks of your place')}
        <div className="grid mt-2 gap-2 grid-cols-2 md:grid-cols-3 lg:grid-cols-6">
          <Perks selected={perks} onChange={setPerks} />
        </div>
        {preInput('Extra info','house rules, etc')}
        <textarea value={extraInfo} className="textarea-field" onChange={ev => setExtraInfo(ev.target.value)} />
        ...
        <div className="grid gap-2 grid-cols-2 md:grid-cols-4">
          ...
          <div>
            ...
            <input type="number" className="input-field" value={maxGuests}
        ...
        <button className="submit-button mt-4" type="submit">Save</button>
      </form>
    </div>
  );
}
